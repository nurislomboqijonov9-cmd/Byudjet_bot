...
api.get('/atchot', (req, res) => {
  ...
  // Davr ichidagi kirim/chiqim — kategoriya bo'yicha
  const catIn = {}, catOut = {};
  const methIn = mkMethodObj(), methOut = mkMethodObj();
  inRange.forEach(r => {
    const m = r.method || 'Naqd so\'m';
    if (r.kind === 'in') {
      catIn[r.category] = (catIn[r.category] || 0) + r.amount;
      if (methIn[m] == null) methIn[m] = 0; methIn[m] += r.amount;
    } else {
      catOut[r.category] = (catOut[r.category] || 0) + r.amount;
  ...
  res.json({
    ...
    catIn: Object.entries(catIn).map(([k, v]) => ({ category: k, amount: v })).sort((a, b) => b.amount - a.amount),
    catOut: Object.entries(catOut).map(([k, v]) => ({ category: k, amount: v })).sort((a, b) => b.amount - a.amount),
